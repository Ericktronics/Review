...
export const oopCards: Flashcard[] = [
...

  {
    ...
    answer:
      "SOLID is five principles that, together, make code easier to extend, test, and change without breaking things.\n\n**S — Single Responsibility Principle**: a class should have **one reason to change**. If a `UserService` validates input, saves to the database, *and* sends welcome emails, any change to email logic forces you to touch the same class as database logic — increasing risk.\n\n**O — Open/Closed Principle**: a module should be **open for extension but closed for modification**. Adding a new payment method should not require editing the existing `PaymentProcessor` class — add a new class that implements the same interface instead (Strategy pattern).\n\n**L — Liskov Substitution Principle**: a subtype must be **substitutable** for its parent type without breaking the program. Classic violation: `Square extends Rectangle` — if `setWidth` on a Square also sets height (to keep it square), code that calls `rect.setWidth(5); rect.setHeight(10)` gets unexpected results.\n\n**I — Interface Segregation Principle**: clients should not be forced to depend on **interfaces they don't use**. Split a fat `IUserService` with 20 methods into focused interfaces like `IUserReader`, `IUserWriter` — services only implement what they need.\n\n**D — Dependency Inversion Principle**: high-level modules should depend on **abstractions**, not concrete implementations. A `UserService` should accept an `IEmailService` interface, not a hardcoded `SendGridService` — making it testable and swappable.",
    code: {
      language: 'typescript',
...
function generateReport(type: string, data: unknown): Buffer {
...

  {
    ...
    answer:
      "The four pillars describe how OOP organizes code to make it easier to understand, change, and reuse.\n\n**1. Encapsulation** — bundling data (fields) and the behaviour that operates on it (methods) into one class, and hiding internal details with `private`/`protected`. External code cannot accidentally break internals it was never supposed to touch. *Analogy: a car's engine is encapsulated — you use the steering wheel, not the fuel injector directly.*\n\n**2. Abstraction** — exposing only what callers need. Interfaces and abstract classes define *what* something does, not *how*. Callers depend on the contract, so the implementation can change without breaking them.\n\n**3. Inheritance** — a child class inherits all non-private members of its parent, enabling code reuse via an 'is-a' relationship (`Dog` is-an `Animal`). Use sparingly — prefer composition for 'has-a' relationships.\n\n**4. Polymorphism** — the same method call behaves differently depending on the actual object type at runtime. A `Shape.area()` call on a `Circle` returns `πr²`; on a `Square` it returns `side²`. The caller doesn't need to know which type it is.",
    code: {
      language: 'typescript',
