...
app.use('/api', (_req, res, next) => {
...

  {
    ...
    answer:
      "**Authentication (AuthN)** answers: *Who are you?* It verifies the user's identity.\n- How: password + username, OAuth token, API key, biometrics\n- Result: 'You are Alice, user ID 42'\n\n**Authorization (AuthZ)** answers: *What are you allowed to do?* It checks whether the already-identified user has permission to perform a specific action.\n- How: role checks, access control lists (ACLs), policy engines (Casbin, OPA)\n- Result: 'Alice has the `admin` role, so she can delete posts'\n\n**Analogy**: authentication is showing your ID at the door of a club. Authorization is the bouncer checking your wristband to see which areas you can enter.\n\n**The order always matters**: you must authenticate first, then authorize.\n- `401 Unauthorized` — means not authenticated (confusingly named by the HTTP spec)\n- `403 Forbidden` — means authenticated but not authorized\n\n**Most common mistake**: returning `401` with the message 'You are not authorized' mixes up the two terms and confuses API clients. Use `401` for missing/invalid credentials, `403` for insufficient permissions.",
  },

  {
    ...
    answer:
      "**A JWT (JSON Web Token) is a compact, self-contained token** that carries user information (called *claims*) encoded and signed, so the server can trust the data without looking it up in a database on every request.\n\n**Structure**: three Base64-URL segments separated by `.` → `header.payload.signature`\n\n- **Header** — algorithm + token type: `{ \"alg\": \"HS256\", \"typ\": \"JWT\" }`\n- **Payload** — claims (the actual data): `{ \"sub\": \"user-42\", \"role\": \"admin\", \"exp\": 1716000000 }`\n- **Signature** — `HMACSHA256(base64(header) + \".\" + base64(payload), secret)` — if anyone tampers with the payload, the signature becomes invalid\n\n**Critical point**: the payload is Base64-encoded, **not encrypted** — anyone who has the token can decode and read it. Never put passwords, credit card numbers, or other sensitive data in a JWT payload.\n\n**How verification works**: the server re-computes the signature from the header and payload using its secret, then compares it to the signature in the token. If they match, the payload has not been tampered with. The server also checks `exp` (expiry), `iss` (issuer), and `aud` (audience) claims.\n\n**Why this matters**: because verification only requires the secret (no DB call), JWTs enable stateless authentication that scales horizontally.",
    code: {
      language: 'typescript',
...
try {
...

  {
    ...
    answer:
      "**Session-based auth (stateful)**:\n1. On login, the server creates a session record in Redis/DB and sends back a `session_id` in a **httpOnly cookie**\n2. On every subsequent request, the server looks up the `session_id` to retrieve user data\n\n- **Pro**: instant revocation — delete the session record and the user is logged out immediately\n- **Pro**: the sensitive data (user details) stays on the server; the client only holds an opaque ID\n- **Con**: stateful — requires a shared session store (Redis) when running multiple server instances; scales less cleanly\n\n**Token-based auth (stateless)**:\n1. On login, the server creates a signed **JWT** containing the user's claims and sends it to the client\n2. On every subsequent request, the client sends the JWT; the server verifies the signature — **no database lookup needed**\n\n- **Pro**: truly stateless — any server instance can verify any token without shared state; scales horizontally trivially\n- **Pro**: works well for microservices and mobile apps where the client controls storage\n- **Con**: revocation is hard — once issued, the token is valid until expiry. You must use short TTLs + refresh token rotation to limit exposure.\n\n**When to use which**:\n\n| Scenario | Use |\n|---|---|\n| Traditional web app, server-rendered HTML | Sessions |\n| SPA (React/Vue), mobile app | JWT |\n| Microservices, API-to-API | JWT |\n| Need instant revocation (logout, ban user) | Sessions (or JWT + denylist) |",
    code: {
      language: 'typescript',
...
await prisma.$queryRaw\`SELECT * FROM users WHERE name = \${req.query.name}\`;`,
...

  {
    ...
    answer:
      "**MD5 and SHA-256 are general-purpose hash functions designed to be fast** — a GPU can compute billions of them per second. That speed is an asset for checksums, but a critical vulnerability for passwords: an attacker who steals your `users` table can try billions of password guesses per second.\n\n**bcrypt is intentionally slow**, and that's the entire point:\n\n- **Configurable cost factor** — each increment doubles the compute time. Cost 10 takes ~100 ms; cost 12 takes ~300 ms. As GPUs get faster, you raise the factor.\n- **Built-in per-password salt** — a random salt is generated automatically for every password. Two users with the same password get different hashes. This defeats rainbow table attacks (pre-computed hash lookup tables).\n- **`bcrypt.compare()` is constant-time** — it always takes the same amount of time regardless of where the strings differ, preventing timing attacks.\n\n**Rule of thumb**: cost factor 12 is the 2025 baseline. At ~300 ms per hash, it is fast enough for login UX but would take an attacker thousands of years to crack a strong password database.\n\n**Alternatives**: Argon2id (winner of the 2015 Password Hashing Competition) is preferred for new projects — it is harder to run on specialized hardware (GPUs/ASICs).",
    code: {
      language: 'typescript',
...
await verifyPassword('wrong',     hash); // false`,
...

  {
    ...
    answer:
      "The **OWASP Top 10** is the authoritative list of the most critical web application security risks. These are the items most relevant to a Node.js API:\n\n**A01 — Broken Access Control** (most common, most dangerous)\nNot verifying that the logged-in user is allowed to access the specific resource they're requesting. Fix: always scope queries to `req.user.id` — never trust the ID in the URL alone.\n\n**A02 — Cryptographic Failures**\nStoring passwords in plain text or using weak hashing (MD5, SHA-1). No TLS on the API. Fix: bcrypt/Argon2 for passwords, HTTPS everywhere, AES-256 for sensitive data at rest.\n\n**A03 — Injection**\nSQL injection, NoSQL injection, command injection — all caused by concatenating user input into queries or commands. Fix: always use parameterized queries; validate input with Zod/Joi; never use string interpolation in SQL.\n\n**A05 — Security Misconfiguration**\nDefault credentials left unchanged, verbose stack traces exposed in API error responses, `CORS: *` on APIs with authentication, missing security headers (`X-Content-Type-Options`, `HSTS`). Fix: `helmet` middleware covers most HTTP security headers.\n\n**A06 — Vulnerable and Outdated Components**\nOutdated npm packages with known CVEs in your `node_modules`. Fix: run `npm audit` in CI, use Dependabot/Renovate for automated update PRs.\n\n**A07 — Identification and Authentication Failures**\nNo expiry on JWT tokens, weak or hardcoded `JWT_SECRET`, no rate limiting on `/login` (enables brute-force). Fix: short-lived tokens (15 min), strong secrets from a secrets manager, rate limit auth endpoints.\n\n**Quick wins that cover multiple categories**: `helmet` (A05), `express-rate-limit` (A07), Zod validation (A03), `npm audit` in CI (A06).",
    code: {
      language: 'typescript',
      snippet: `import helmet from 'helmet';
