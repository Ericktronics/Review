...
await db.$transaction(async (tx) => {
...

  {
    ...
    answer:
      '| | SQL (Relational) | NoSQL |\n|---|---|---|\n| Schema | Fixed, enforced by the DB | Flexible, schema-on-read |\n| Data model | Tables, rows, columns | Documents, key-value, graph, wide-column |\n| Relationships | JOINs, foreign keys | Embedded docs or app-level joins |\n| Transactions | ACID by default | Varies (MongoDB 4+ supports multi-doc ACID) |\n| Scaling | Vertical; read replicas | Horizontal sharding built-in |\n| Examples | PostgreSQL, MySQL, SQLite | MongoDB, Redis, DynamoDB, Cassandra |\n\n**Choose SQL when**: data has real foreign-key relationships (users → orders → products), **referential integrity must be enforced at the DB level** (no orphaned records without application logic), ACID transactions are required (payments, inventory, medical records), or the schema is stable and well-understood.\n\n**Choose NoSQL when**: schema changes rapidly or is document-shaped, you need horizontal scale from day one, or the problem fits a specialised model (Redis for key-value/cache, Cassandra for time-series, Neo4j for graphs).',
  },
